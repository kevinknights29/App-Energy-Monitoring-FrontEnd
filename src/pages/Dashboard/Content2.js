...
const Content2 = () => {
  ...
  const peak_power_vals = vals.filter(vals => vals.unit === "Wp").slice(-25)
  const data = {
    labels: peak_power_vals.map(peak_power_vals => { return peak_power_vals.datetime }),
    datasets: [
      {
        label: 'Potencia Pico (W)',
        backgroundColor: 'rgb(255, 255, 0)',
        borderColor: 'rgb(125, 125, 125)',
        data: peak_power_vals.map(peak_power_vals => { return peak_power_vals.value })
      }
    ]
  }
  const options = {
